...
class OpenWordPressSEOWidget {
	...
	public function print_dashboard_widget() {
		...
		echo '<tr><td><strong>'.__('Overall SEO score', 'open-wp-seo').'</strong></td><td><strong>'.$this->get_overall_seo_status_score_text().'</strong></td></tr>';

		echo '<tr><td>'.__('Frontpage SEO score', 'open-wp-seo').'</td><td>'.$this->get_frontpage_seo_status_text().'</td></tr>';

		echo '<tr><td>'.__('Articles SEO score', 'open-wp-seo').'</td><td>'.$this->get_posts_seo_status_text('post').'</td></tr>';

		echo '<tr><td>'.__('Pages SEO score', 'open-wp-seo').'</td><td>'.$this->get_posts_seo_status_text('page').'</td></tr>';

		echo '<tr><td>'.__('Sitemap updated', 'open-wp-seo').'</td><td>'.$this->get_sitemap_status().'</td></tr>';

		echo '</table>';

		if ($indexing_denied) {
			printf('<table><tr><td class="icon"><span class="dashicons dashicons-warning"></span></td><td>'.__('Search engines are told not to index this site. Change the setting in <a href="%s">Reading</a> > Search Engine Visibility.', 'open-wp-seo').'</td></tr></table>', get_admin_url().'/options-reading.php');
		}
	...
	private function get_overall_seo_status_score_text() {
		...
		$score_text = __('Low', 'open-wp-seo') . ' <span class="dashicons dashicons-thumbs-down small-icon"></span>';

		if ($score > 0.9) {
			$score_text = __('High', 'open-wp-seo') . ' <span class="dashicons dashicons-thumbs-up small-icon"></span>';
		}
		else if ($score > 0.6) {
			$score_text = __('OK', 'open-wp-seo');
		}
	...
	private function get_sitemap_status() {
		...
		if ($sitemap_enabled !== OpenWordPressSEO::OPTION_ON) {
			return '<span class="open-wp-seo-fail">'. sprintf(__('Not enabled. <a href="%s">Enable here</a>', 'open-wp-seo'), $settings_url) . '</span>';
		}

		if ($sitemap_updated === OpenWordPressSEO::STATUS_ERROR) {
			return '<span class="open-wp-seo-fail">' . __('Error', 'open-wp-seo') . '</span>';
		}
